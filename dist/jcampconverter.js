...
(function webpackUniversalModuleDefinition(root, factory) {
  ...
  else if (typeof exports === 'object') exports['JcampConverter'] = factory();
  else root['JcampConverter'] = factory();
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        function getConverter() {
          ...
          function convert(jcamp, options) {
            ...
            for (let i = 0; i < ldrs.length; i++) {
              ...
              let canonicDataLabel = dataLabel
                .replace(/[_ -]/g, '')
                .toUpperCase();
              ...
              if (canonicDataLabel === 'DATATABLE') {
                ...
                if (endLine > 0) {
                  ...
                  if (infos[1] && infos[1].indexOf('PEAKS') > -1) {
                    ...
                    spectrum.deltaX =
                      (spectrum.lastX - spectrum.firstX) /
                      (spectrum.nbPoints - 1);
                  }
                }
              }
              ...
              if (canonicDataLabel === 'TITLE') {
                ...
                ntuples.vardim = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
              } else if (canonicDataLabel === 'UNITS') {
                ntuples.units = dataValue.split(ntuplesSeparator);
              } else if (canonicDataLabel === 'FACTOR') {
                ntuples.factor = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
              } else if (canonicDataLabel === 'FIRST') {
                ntuples.first = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
              } else if (canonicDataLabel === 'LAST') {
                ntuples.last = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
              } else if (canonicDataLabel === 'MIN') {
                ntuples.min = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
              } else if (canonicDataLabel === 'MAX') {
                ntuples.max = convertToFloatArray(
                  dataValue.split(ntuplesSeparator),
                );
                ...
                let pageSymbolIndex = ntuples.symbol.indexOf(
                  spectrum.pageSymbol,
                );
                let unit = '';
              ...
              if (canonicDataLabel.match(options.keepRecordsRegExp)) {
                let label = options.canonicDataLabels
                  ? canonicDataLabel
                  : dataLabel;
              ...
              for (
                ...
                for (let povar = 0; povar < nbPovars - 1; povar++) {
                  ...
                  if (isOver0 !== isOver1 && isOver0 !== isOver2) {
                    pAx =
                      povar +
                      (lineZValue - povarHeight0) /
                        (povarHeight1 - povarHeight0);
                    pAy = iSubSpectra;
                    pBx = povar;
                    pBy =
                      iSubSpectra +
                      (lineZValue - povarHeight0) /
                        (povarHeight2 - povarHeight0);
                  ...
                  if (isOver3 !== isOver1 && isOver3 !== isOver2) {
                    ...
                    pAy =
                      iSubSpectra +
                      1 -
                      (lineZValue - povarHeight3) /
                        (povarHeight1 - povarHeight3);
                    pBx =
                      povar +
                      1 -
                      (lineZValue - povarHeight3) /
                        (povarHeight2 - povarHeight3);
                    pBy = iSubSpectra + 1;
                    lines.push(pAx * dx + x0);
                    lines.push(pAy * dy + y0);
                    lines.push(pBx * dx + x0);
                  ...
                  if (isOver1 !== isOver2) {
                    pAx =
                      (povar +
                        1 -
                        (lineZValue - povarHeight1) /
                          (povarHeight2 - povarHeight1)) *
                        dx +
                      x0;
                    pAy =
                      (iSubSpectra +
                    ...
                    if (isOver2 !== isOver0) {
                      ...
                      pBy =
                        iSubSpectra +
                        1 -
                        (lineZValue - povarHeight2) /
                          (povarHeight0 - povarHeight2);
                      lines.push(pAx);
                      lines.push(pAy);
                      lines.push(pBx * dx + x0);
                      lines.push(pBy * dy + y0);
              ...
              if (inComment) {
                ...
                if (ascii <= 57 && ascii >= 48) {
                  ...
                  if (decimalPosition > 0) {
                    currentValue +=
                      (ascii - 48) / Math.pow(10, decimalPosition++);
                  } else {
                    currentValue *= 10;
                  ...
                  if (inValue) {
                    ...
                    if (newLine) {
                      ...
                      if (skipFirstValue) {
                        ...
                        if (isDifference) {
                          lastDifference = isNegative
                            ? 0 - currentValue
                            : currentValue;
                          isLastDifference = true;
                          isDifference = false;
                        } else if (!isDuplicate) {
                          lastValue = isNegative
                            ? 0 - currentValue
                            : currentValue;
                        }
            ...
            for (let i = 1; i < lines.length; i++) {
              ...
              if (values.length % 2 === 0) {
                for (let j = 0; j < values.length; j = j + 2) {
                  ...
                  currentData.push(
                    parseFloat(values[j + 1]) * spectrum.yFactor,
                  );
                }
              } else {
        ...
        function createWorker() {
          let workerURL = URL.createObjectURL(
            new Blob(
              [
                'var getConverter ='.concat(
                  getConverter.toString(),
                  ';var convert = getConverter(); onmessage = function (event) { var data = JSON.parse(event.data); postMessage(JSON.stringify({stamp: data.stamp, output: convert(data.input, data.options)})); };',
                ),
              ],
              {
