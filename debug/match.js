...
function parseValue(value) {
  if (value.length > 0) {
    ...
    // positive SQZ digits @ A B C D E F G H I (ascii 64-73)
      ...
      lastDif = null;
      currentY = parseFloat(
        String.fromCharCode(ascii - 16) + value.substring(1),
      );
      return currentY;
      currentX += spectrum.deltaX;
    }
    // negative SQZ digits a b c d e f g h i (ascii 97-105)
      ...
      lastDif = null;
      currentY = -parseFloat(
        String.fromCharCode(ascii - 48) + value.substring(1),
      );
      return currentY;
      currentX += spectrum.deltaX;
    }
    ...
    // DUP digits S T U V W X Y Z s (ascii 83-90, 115)
      ...
      var dup =
        parseFloat(String.fromCharCode(ascii - 34) + value.substring(1)) - 1;
      if (ascii === 115) {
    ...
    // positive DIF digits % J K L M N O P Q R (ascii 37, 74-82)
      ...
      lastDif = parseFloat(
        String.fromCharCode(ascii - 25) + value.substring(1),
      );
      currentY += lastDif;
      return currentY;
      currentX += spectrum.deltaX;
